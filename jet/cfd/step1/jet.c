...
event write_boundaryfile (i++) {  
  ...
  const char *fname = "boundary.csv";  

  static FILE *bf = fopen (fname, "w");
  ...
  if (i == 0) {    
    ...
    fprintf (bf, "i,t,dt,x,y,ux,uy,f,p\n");    
  }
    ...
    for (double y = -.5*domain_height+boundary_resolution; 
      ...
      fprintf (bf, "%d,%.14g,%.14g,%.14g,%.14g,%.14g,%.14g,%.14g,%.14g\n", i, t, dt, measurement_x, y,  interpolate(u.x,measurement_x,y),
               interpolate(u.y,measurement_x,y), interpolate(ff,measurement_x,y), interpolate(p,measurement_x,y));
    }
...
event write_batch_prediction_inputs (t = measurement_time) {  // t = end
  ...
  // ux
  const char *fname = "ux_batch_prediction_inputs.csv";  
  bpif = fopen (fname, "w");
  fprintf (bpif, "dt,y,ux\n");    
  for (double y = -.5*domain_height+boundary_resolution; 
              y <= .5*domain_height-boundary_resolution; 
              y += boundary_resolution) {      
    fprintf (bpif, "%.14g,%.14g,%.14g\n", dt, y, interpolate(u.x,measurement_x,y));
  }
  fflush (bpif);
  fclose (bpif);
  
  // uy
  fname = "uy_batch_prediction_inputs.csv";  
  bpif = fopen (fname, "w");    
  fprintf (bpif, "dt,y,uy\n");    
  for (double y = -.5*domain_height+boundary_resolution; 
              y <= .5*domain_height-boundary_resolution; 
              y += boundary_resolution) {      
    fprintf (bpif, "%.14g,%.14g,%.14g\n", dt, y, interpolate(u.y,measurement_x,y));
  }
  fflush (bpif);
  fclose (bpif);
  
  // f
  fname = "f_batch_prediction_inputs.csv";  
  bpif = fopen (fname, "w");    
  fprintf (bpif, "dt,y,f\n");    
  for (double y = -.5*domain_height+boundary_resolution; 
              y <= .5*domain_height-boundary_resolution; 
              y += boundary_resolution) {      
    fprintf (bpif, "%.14g,%.14g,%.14g\n", dt, y, interpolate(ff,measurement_x,y));
  }
  fflush (bpif);
  fclose (bpif);
