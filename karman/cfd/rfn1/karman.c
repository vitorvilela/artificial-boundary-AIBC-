...
event boundaryfile (i++) {  
  ...
  if (i == 0) {
    ...
    for (double y = -cutted_y_length+boundary_resolution; y <= cutted_y_length-boundary_resolution; y += boundary_resolution)    
      fprintf (bf, "%d, %g, %g, %g, %g, %g, %g, %g, %g\n", i, t, dt, measurement, y, interpolate(u.x,measurement,y), interpolate(u.y,measurement,y), interpolate(f,measurement,y), interpolate(p,measurement,y));
  }   
